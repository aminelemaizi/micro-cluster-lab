...
RUN wget -P /tmp/ https://archive.apache.org/dist/hadoop/common/hadoop-3.2.0/hadoop-3.2.0.tar.gz
RUN tar xvf /tmp/hadoop-3.2.0.tar.gz -C /tmp && \
	mv /tmp/hadoop-3.2.0 /opt/hadoop

RUN wget -P /tmp/ https://downloads.apache.org/spark/spark-3.2.0/spark-3.2.0-bin-hadoop3.2.tgz
RUN tar xvf /tmp/spark-3.2.0-bin-hadoop3.2.tgz -C /tmp &&  \
